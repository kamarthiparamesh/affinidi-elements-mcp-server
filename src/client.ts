...
function printHelp(): void {
    ...
    console.log('  create-note [title] [content]  - Call the greet tool');
    ...
    readline.question('\n> ', async (input) => {
        ...
        try {
            switch (command) {
                ...
                case 'create-note':
                    await callCreateNoteTool(args[1], args[2]);
                    break;
                ...
                case 'read-resource':
                    if (args.length < 2) {
                        console.log('Usage: read-resource [id]');
                    }
                    else {
                        const id = args[1];
                        await readResources(id);
                    }
                    break;
                case 'help':
                    printHelp();
...
async function callCreateNoteTool(title: string, content: string): Promise<void> {
    await callTool('create_note', { title, content });
}
...
async function readResources(id: string): Promise<void> {
    if (!client) {
        console.log('Not connected to server.');
        return;
    }
